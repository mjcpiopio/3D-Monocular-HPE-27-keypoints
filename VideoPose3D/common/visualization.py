...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
                     limit=-1, downsample=1, size=6, input_video_path=None, input_video_skip=0,display_mode='mp4' ):
    ...
    elif display_mode == 'mp4':
        Writer = writers['ffmpeg']
        writer = Writer(fps=fps, metadata={}, bitrate=bitrate,codec="libx264", extra_args=['-pix_fmt', 'yuv420p'])
        current_directory = os.getcwd()

        # Construct the output folder path inside the VideoPose3D directory
        anim.save(output, writer=writer)
        plt.close(anim._fig)
        return output
    else:
        raise ValueError('Unsupported display mode. Use "interactive", "html", or "mp4".')
    plt.close()
